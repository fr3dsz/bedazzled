...
export function EmptyState({ icon: Icon, title, description, action, className, size = "default" }: EmptyStateProps) {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center text-center", config.container, className)}>
      <div className={cn("flex items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800", config.iconWrapper)}>
        <Icon className={cn("text-zinc-400", config.icon)} />
      </div>
      <h2 className={cn("font-semibold text-zinc-900 dark:text-zinc-100", config.title)}>{title}</h2>
      {description && <p className={cn("max-w-sm text-zinc-500 dark:text-zinc-400", config.description)}>{description}</p>}
      {ActionButton}
    </div>
  );
}
