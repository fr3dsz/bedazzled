...
function FeaturedToggleContent(handle: FeaturedToggleProps) {
  ...
  return (
    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => editFeatured(!isFeatured)} title={isFeatured ? "Remove from featured" : "Add to featured"}>
      <Star className={cn("h-4 w-4 transition-colors", isFeatured ? "fill-amber-400 text-amber-400" : "text-zinc-300 dark:text-zinc-600")} />
    </Button>
  );
}
