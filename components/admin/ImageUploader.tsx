...
import { useDocument, useEditDocument, useClient, type DocumentHandle } from "@sanity/sdk-react";
...
function ImageUploaderContent(handle: DocumentHandle) {
  ...
  const handleRemoveImage = (keyToRemove: string) => {
    const updatedImages = currentImages.filter((img) => img._key !== keyToRemove);
      ...
      {currentImages.length > 0 ? (
        <div className="grid grid-cols-2 gap-3">
          {currentImages.map((image, index) => (
            <ImageThumbnail key={image._key} image={image} index={index} isFirst={index === 0} onRemove={() => handleRemoveImage(image._key)} onMoveUp={() => handleMoveImage(index, index - 1)} onMoveDown={() => handleMoveImage(index, index + 1)} canMoveUp={index > 0} canMoveDown={index < currentImages.length - 1} />
          ))}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-200 py-8 dark:border-zinc-700">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500">Click upload to add product images</p>
        </div>
      )}

      {currentImages.length > 0 && <p className="text-xs text-zinc-500 dark:text-zinc-400">First image is the main product image. Drag to reorder.</p>}
    </div>
  );
}
...
function ImageThumbnail({ image, isFirst, onRemove, onMoveUp, onMoveDown, canMoveUp, canMoveDown }: ImageThumbnailProps) {
  ...
  return (
    <div className={cn("group relative aspect-square overflow-hidden rounded-lg bg-zinc-100 dark:bg-zinc-800", isFirst && "ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-zinc-900")}>
      {imageUrl ? (
      ...
      {/* First image badge */}
      {isFirst && <div className="absolute left-2 top-2 rounded bg-blue-500 px-1.5 py-0.5 text-xs font-medium text-white">Main</div>}

      {/* Actions overlay */}
      <div className="absolute inset-0 flex items-center justify-center gap-1 bg-black/50 opacity-0 transition-opacity group-hover:opacity-100">
        {/* Reorder buttons */}
        <div className="flex flex-col gap-1">
          <Button type="button" variant="secondary" size="icon" className="h-7 w-7" onClick={onMoveUp} disabled={!canMoveUp}>
            <ChevronUp className="h-4 w-4" />
          </Button>
          <Button type="button" variant="secondary" size="icon" className="h-7 w-7" onClick={onMoveDown} disabled={!canMoveDown}>
