  ...
  return (
    <TableRow className="group transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
      ...
      <TableCell className="py-3 sm:py-4">
        <Link href={`/admin/orders/${handle.documentId}`} className="block">
          <div className="flex items-center justify-between gap-2 sm:block">
            <span className="font-medium text-zinc-900 dark:text-zinc-100">#{formatOrderNumber(data.orderNumber)}</span>
            {/* Mobile: Total inline */}
            <span className="font-medium text-zinc-900 dark:text-zinc-100 sm:hidden">{formatPrice(data.total)}</span>
          </div>
      ...
      <TableCell className="hidden py-4 text-zinc-500 dark:text-zinc-400 sm:table-cell">
        <Link href={`/admin/orders/${handle.documentId}`} className="block truncate">
          {data.email}
        </Link>
      </TableCell>
      ...
      <TableCell className="py-3 sm:py-4">
        <Link href={`/admin/orders/${handle.documentId}`} className="flex justify-center sm:justify-start">
