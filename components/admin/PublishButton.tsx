...
import { useApplyDocumentActions, useDocument, publishDocument, discardDocument, type DocumentHandle } from "@sanity/sdk-react";
...
function PublishButtonContent({ variant = "default", size = "default", ...handle }: PublishButtonProps) {
  ...
  return (
    <Button variant={variant} size={size} onClick={handlePublish} disabled={isPublishing} className="min-w-[140px]">
      {isPublishing ? (
        <>
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Button variant="destructive" size={size} onClick={handleRevert} disabled={isReverting}>
          {isReverting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Undo2 className="h-4 w-4" />}
        </Button>
      </TooltipTrigger>
      <TooltipContent>
        <p>Discard changes</p>
