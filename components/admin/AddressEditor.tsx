...
function AddressField({ handle, field, label, placeholder }: { handle: DocumentHandle; field: string; label: string; placeholder?: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <Input id={field} value={(value as string) ?? ""} onChange={(e) => editField(e.target.value)} placeholder={placeholder} className="h-9" />
    </div>
  );
}
