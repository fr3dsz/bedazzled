...
// Query: *[  _type == "customer"  && email == $email][0]{  _id,  email,  name,  clerkUserId,  paymongoCustomerId,  createdAt}
...
// Source: sanity/queries/customers.ts
// Variable: CUSTOMER_BY_PAYMONGO_ID_QUERY
// Query: *[  _type == "customer"  && paymongoCustomerId == $paymongoCustomerId][0]{  _id,  email,  name,  clerkUserId,  paymongoCustomerId,  createdAt}
export type CUSTOMER_BY_PAYMONGO_ID_QUERY_RESULT = {
  _id: string;
  email: string | null;
  name: string | null;
  clerkUserId: string | null;
...
// Query: *[  _type == "order"  && _id == $id][0] {  _id,  orderNumber,  clerkUserId,  email,  items[]{    _key,    quantity,    priceAtPurchase,    product->{      _id,      name,      "slug": slug.current,      "image": images[0]{        asset->{          _id,          url        }      }    }  },  total,  status,  address{    name,    line1,    line2,    barangay,    city,    province,    zipCode,    country  },  paymongoPaymentId,  paymentMethod,  createdAt}
...
// Query: *[  _type == "order"] | order(createdAt desc) [0...$limit] {  _id,  orderNumber,  email,  total,  status,  paymentMethod,  createdAt}
...
// Source: sanity/queries/orders.ts
// Variable: ORDER_BY_PAYMONGO_PAYMENT_ID_QUERY
// Query: *[  _type == "order"  && paymongoPaymentId == $paymongoPaymentId][0]{ _id }
export type ORDER_BY_PAYMONGO_PAYMENT_ID_QUERY_RESULT = {
  _id: string;
} | null;

// Source: sanity/queries/products.ts
// Variable: ALL_PRODUCTS_QUERY
// Query: *[  _type == "product"] | order(name asc) {  _id,  name,  "slug": slug.current,  description,  price,  "images": images[]{    _key,    asset->{      _id,      url    },    hotspot  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock,  featured}
...
// Query: *[  _type == "product"  && category->slug.current == $categorySlug] | order(name asc) {  _id,  name,  "slug": slug.current,  price,  "image": images[0]{    asset->{      _id,      url    },    hotspot  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && slug.current == $slug][0] {  _id,  name,  "slug": slug.current,  description,  price,  "images": images[]{    _key,    asset->{      _id,      url    },    hotspot  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock,  featured}
...
// Query: *[  _type == "product"  && (    name match $searchQuery + "*"    || description match $searchQuery + "*"  )] | score(  boost(name match $searchQuery + "*", 3),  boost(description match $searchQuery + "*", 1)) | order(_score desc) {  _id,  _score,  name,  "slug": slug.current,  price,  "image": images[0]{    asset->{      _id,      url    },    hotspot  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && ($categorySlug == "" || category->slug.current == $categorySlug)  && ($color == "" || $color in colors)  && ($shape == "" || shape == $shape)  && ($minPrice == 0 || price >= $minPrice)  && ($maxPrice == 0 || price <= $maxPrice)  && ($searchQuery == "" || name match $searchQuery + "*" || description match $searchQuery + "*")  && ($inStock == false || stock > 0)] | order(name asc) {  _id,  name,  "slug": slug.current,  price,  "images": images[0...4]{    _key,    asset->{      _id,      url    }  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && ($categorySlug == "" || category->slug.current == $categorySlug)  && ($color == "" || $color in colors)  && ($shape == "" || shape == $shape)  && ($minPrice == 0 || price >= $minPrice)  && ($maxPrice == 0 || price <= $maxPrice)  && ($searchQuery == "" || name match $searchQuery + "*" || description match $searchQuery + "*")  && ($inStock == false || stock > 0)] | order(price asc) {  _id,  name,  "slug": slug.current,  price,  "images": images[0...4]{    _key,    asset->{      _id,      url    }  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && ($categorySlug == "" || category->slug.current == $categorySlug)  && ($color == "" || $color in colors)  && ($shape == "" || shape == $shape)  && ($minPrice == 0 || price >= $minPrice)  && ($maxPrice == 0 || price <= $maxPrice)  && ($searchQuery == "" || name match $searchQuery + "*" || description match $searchQuery + "*")  && ($inStock == false || stock > 0)] | order(price desc) {  _id,  name,  "slug": slug.current,  price,  "images": images[0...4]{    _key,    asset->{      _id,      url    }  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && ($categorySlug == "" || category->slug.current == $categorySlug)  && ($color == "" || $color in colors)  && ($shape == "" || shape == $shape)  && ($minPrice == 0 || price >= $minPrice)  && ($maxPrice == 0 || price <= $maxPrice)  && ($searchQuery == "" || name match $searchQuery + "*" || description match $searchQuery + "*")  && ($inStock == false || stock > 0)] | order(_createdAt desc) {  _id,  name,  "slug": slug.current,  price,  "images": images[0...4]{    _key,    asset->{      _id,      url    }  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && ($categorySlug == "" || category->slug.current == $categorySlug)  && ($color == "" || $color in colors)  && ($shape == "" || shape == $shape)  && ($minPrice == 0 || price >= $minPrice)  && ($maxPrice == 0 || price <= $maxPrice)  && ($searchQuery == "" || name match $searchQuery + "*" || description match $searchQuery + "*")  && ($inStock == false || stock > 0)] | score(  boost(name match $searchQuery + "*", 3),  boost(description match $searchQuery + "*", 1)) | order(_score desc, name asc) {  _id,  name,  "slug": slug.current,  price,  "images": images[0...4]{    _key,    asset->{      _id,      url    }  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock}
...
// Query: *[  _type == "product"  && (    $searchQuery == ""    || name match $searchQuery + "*"    || description match $searchQuery + "*"    || category->title match $searchQuery + "*"  )  && ($categorySlug == "" || category->slug.current == $categorySlug)  && ($shape == "" || shape == $shape)  && ($color == "" || $color in colors)  && ($minPrice == 0 || price >= $minPrice)  && ($maxPrice == 0 || price <= $maxPrice)] | order(name asc) [0...20] {  _id,  name,  "slug": slug.current,  description,  price,  "image": images[0]{    asset->{      _id,      url    }  },  category->{    _id,    title,    "slug": slug.current  },  shape,  colors,  stock,  featured}
...
// Query: *[  _type == "order"  && createdAt >= $startDate  && !(_id in path("drafts.**"))] | order(createdAt desc) {  _id,  orderNumber,  total,  status,  paymentMethod,  createdAt,  "itemCount": count(items),  items[]{    quantity,    priceAtPurchase,    "productName": product->name,    "productId": product->_id  }}
...
// Source: sanity/queries/stats.ts
// Variable: PAYMENT_METHOD_DISTRIBUTION_QUERY
// Query: {  "gcash": count(*[_type == "order" && paymentMethod == "gcash" && !(_id in path("drafts.**"))]),  "maya": count(*[_type == "order" && paymentMethod == "maya" && !(_id in path("drafts.**"))]),  "card": count(*[_type == "order" && paymentMethod == "card" && !(_id in path("drafts.**"))]),  "bank_transfer": count(*[_type == "order" && paymentMethod == "bank_transfer" && !(_id in path("drafts.**"))])}
export type PAYMENT_METHOD_DISTRIBUTION_QUERY_RESULT = {
  gcash: number;
  maya: number;
  card: number;
...
// Query: *[_type == "product"] {  _id,  name,  price,  stock,  shape,  "category": category->title}
...
// Query: *[  _type == "order"  && status == "paid"  && !(_id in path("drafts.**"))] | order(createdAt asc) {  _id,  orderNumber,  total,  createdAt,  email,  paymentMethod,  "itemCount": count(items)}
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    '*[\n  _type == "order"\n  && paymongoPaymentId == $paymongoPaymentId\n][0]{ _id }': ORDER_BY_PAYMONGO_PAYMENT_ID_QUERY_RESULT;
